# main.py - Capacity Management Dashboard (UI & usability improvements)
...
# - NEW: "Sync Tickets" button pulls hardware-request tickets (via SharePoint) into
#   Recent Events automatically. See sync_sharepoint_to_dashboard.py for the sync logic
#   and sync_tickets_to_sharepoint.py for the upstream Azure DevOps -> SharePoint step.
#
# Requirements:
# - config.py in same folder exporting DB_CONFIG, TOTAL_CAPACITY, RESERVED_PERCENT
#   TOTAL_CAPACITY: {"CPU": cores, "RAM": GB, "STORAGE": TB}
# - pipeline_config.py (optional) exporting SHAREPOINT_CONFIG - see pipeline_config.example.py.
#   If it's missing, the "Sync Tickets" button is disabled gracefully rather than crashing.
# - Python packages: dash, dash-bootstrap-components, pandas, sqlalchemy, psycopg2-binary,
#   plotly, requests, msal (msal/requests only needed for the ticket sync feature)
...
# Optional: ticket -> dashboard sync. Import defensively so the dashboard still
# runs fine if pipeline_config.py / msal / requests aren't set up yet.
try:
    from sync_sharepoint_to_dashboard import sync_tickets_into_dashboard
    TICKET_SYNC_AVAILABLE = True
except Exception:
    TICKET_SYNC_AVAILABLE = False

    def sync_tickets_into_dashboard():
        raise RuntimeError("Ticket sync not configured (missing pipeline_config.py or dependencies)")
                ...
                dbc.Col(
                    dbc.Card([
                        dbc.CardBody([
                            ...
                            dbc.Button("Upload CSV", id="toggle-upload", color="secondary", className="me-2"),
                            dbc.Button(
                                "Sync Tickets" if TICKET_SYNC_AVAILABLE else "Sync Tickets (not configured)",
                                id="sync-tickets", color="dark",
                                disabled=not TICKET_SYNC_AVAILABLE
                            ),
                            html.P(
                                "Pulls hardware-request tickets (via SharePoint) into Recent Events below.",
                                style={"fontSize": "0.8rem", "color": "#6c757d"}, className="mt-1"
                            ),
                            html.Hr(),
...
def master_callback(submit_n, csv_contents, n_intervals, refresh_n, recent_apply_n, sync_tickets_n,
    ...
    # Sync tickets (SharePoint -> capacity_events), triggered by button
    if triggered_id == "sync-tickets" and sync_tickets_n:
        try:
            created, failed = sync_tickets_into_dashboard()
            if created:
                msg = dbc.Alert(f"Synced {created} ticket(s) into Recent Events ({failed} failed)", color="success")
            else:
                msg = dbc.Alert(f"No new tickets to sync ({failed} failed)", color="info")
            refresh_needed = True
        except Exception as e:
            logger.exception("ticket sync failed")
            msg = dbc.Alert(f"Ticket sync error: {e}", color="danger")
    ...
    if refresh_needed or triggered_id in ("interval", "refresh", "") or triggered_id == "recent-apply":
        ...
        recent_df = pd.read_sql(text("""
            SELECT event_time, assetuniquename, cpu_delta, ram_delta_gb, storage_delta_gb, source
            FROM capacity_events
        ...
        if not inv_df.empty:
            inv_table = dbc.Table.from_dataframe(
                inv_df[["Server Name", "IP Address", "CPU (cores)", "RAM (GB)", "Storage (GB)", "Status"]],
